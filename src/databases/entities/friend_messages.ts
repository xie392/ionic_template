...
export class FriendMessagesEntity {
	...
	@Column({ type: 'int', default: MessageType.TEXT, comment: '消息类型' })
	type!: MessageType

	@Column({ type: 'int', default: MessageStatus.SENT, comment: '消息状态' })
	...
	@Column({
		type: 'text',
		default: '',
		comment: '消息附加信息, 例如图片url, 视频url等, json格式, 前端根据类型解析'
	})
	extra!: string

	@Column({ type: 'bigint', default: 0, comment: '回复消息id' })
	replyId!: number
}
